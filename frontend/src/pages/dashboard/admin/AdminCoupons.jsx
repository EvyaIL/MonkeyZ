...
const AdminCoupons = () => {
  ...
  useEffect(() => {
    if (coupons && coupons.length > 0) {
      const couponsWithIds = coupons.map((coupon, index) => {
        if (coupon.id) return coupon;
        return { ...coupon, id: coupon._id || `temp-coupon-${index}` };
      });
      
      if (couponsWithIds.some((c, i) => JSON.stringify(c) !== JSON.stringify(coupons[i]))) {
  ...
  const handleDeleteCoupon = async (id) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Just refresh once after deletion
        await fetchCoupons();

...
from src.models.key.key import Key, KeyRequest, KeyRespond
...
async def edit_key(key_id:PydanticObjectId,key_request:KeyRequest,key_controller:KeyController = Depends(get_keys_controller_dependency), current_user:User = Depends(get_current_user)):
   key_id = await key_controller.edit_key(key_id, key_request, current_user.username)
   return key_id


@key_router.delete("")
async def delete_key(key_id:PydanticObjectId,key_controller:KeyController = Depends(get_keys_controller_dependency), current_user:User = Depends(get_current_user)):
   key_id = await key_controller.delete_key(key_id, current_user.username)
   return key_id

@key_router.get("/by_product", response_model=list[KeyRespond])
...
@key_router.get("/by_user", response_model=list[KeyRespond])
